...
def SES_model(x):
  ...
  well_being = np.mean(U_array) #total well being  #(previously resilience)

  return resilience, well_being

# equity = np.mean(S) * np.mean(W)   # Well-being is a product of of resource access,S and wage,W. Ranges 0 to 1.
...
for i in range (N):
  if i % 10 == 0:
    print(i)
  Y_resilience[i], Y_well_being = SES_model(param_values[i]) # Do I do this for simulate_SES or run?

exit()

# Perform analysis, calculate sensitivity indices
Si = sobol.analyze(problem, Y_resilience, 
                  print_to_console=True, num_resamples = 10) #1000
